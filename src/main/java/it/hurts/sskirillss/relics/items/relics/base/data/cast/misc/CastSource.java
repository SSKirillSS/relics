...
public enum CastSource implements ICastSource {
    CURIOS {
        @Override
        public Function<Player, List<AbilityReference>> processInventory() {
            return player -> {
                    ...
                    for (Map.Entry<String, ICurioStacksHandler> entry : itemHandler.getCurios().entrySet()) {
                        ...
                        for (int slot = 0; slot < stacksHandler.getSlots(); slot++) {
                            ...
                            for (AbilityData abilityData : relic.getRelicData().getAbilities().getAbilities().values()) {
                                ...
                                if (castData.getType() == CastType.NONE || !castData.getSource().contains(CURIOS))
                                    continue;
            ...
            return player -> {
                ...
                for (int slot = 0; slot < player.getInventory().getContainerSize(); slot++) {
                    ...
                    for (AbilityData abilityData : relic.getRelicData().getAbilities().getAbilities().values()) {
                        ...
                        if (castData.getType() == CastType.NONE || !castData.getSource().contains(INVENTORY))
                            continue;
