...
public class WoolMittenItem extends RelicItem {
    ...
    public void castActiveAbility(ItemStack stack, Player player, String ability, CastType type, CastStage stage) {
        ...
        if (ability.equals("frosty_shroud")) {
            double duration = getAbilityValue(stack, "frosty_shroud", "duration") * 20;
            double radius = getAbilityValue(stack, "frosty_shroud", "radius");
            ...
            if (!level.isClientSide())
                setAbilityCooldown(stack, "frosty_shroud", (int) (getAbilityValue(stack, "frosty_shroud", "cooldown") * 20));
            ...
            if (!level.isClientSide()) {
                for (LivingEntity entity : targets) {
                    ...
                    entity.addEffect(new MobEffectInstance(EffectRegistry.FROSTBITE.get(), (int) duration, 0));
                }
            }
            flag = false;
        }
    ...
    public static class Events {
        ...
        public static void onEntityHit(LivingHurtEvent event) {
            ...
            if (relicStack.isEmpty() || target == null || !(relicStack.getItem() instanceof IRelicItem relic))
                return;
            ...
            double maxCharges = (int) relic.getAbilityValue(relicStack, "cold_touch", "max_charges");
            double meltTime = (int) relic.getAbilityValue(relicStack, "cold_touch", "melt_time");

            COLD_TOUCH_CHARGES.put(target, charges + 1);
            MELT_TIMER.put(target, (int) meltTime);

            if (charges >= maxCharges) {
                target.addEffect(new MobEffectInstance(EffectRegistry.FROSTBITE.get(), 180, 0));
                target.addEffect(new MobEffectInstance(EffectRegistry.STUN.get(), 180, 0));
                ...
                for (LivingEntity nearby : target.level().getEntitiesOfClass(LivingEntity.class, target.getBoundingBox().inflate(3))) {
                    if (nearby != target && !EntityUtils.isAlliedTo(player, nearby)) {
                        ...
                        nearby.addEffect(new MobEffectInstance(EffectRegistry.FROSTBITE.get(), 80, 1));
                        nearby.addEffect(new MobEffectInstance(EffectRegistry.STUN.get(), 180, 0));
                    }
                }
            }
        }
                    ...
                    if (meltTime > 0) {
                        ...
                        if (meltTime <= 1) {
                            ...
                            if (charges > 0) {
                                ...
                                MELT_TIMER.put(entity, (int) relic.getAbilityValue(relicStack, "cold_touch", "melt_time"));
                            }
                        }
                    }
                }
