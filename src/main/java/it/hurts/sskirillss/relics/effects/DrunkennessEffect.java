    ...
    public static class Events {
        ...
        public static void onChatMessage(ServerChatEvent event) {
            ...
            if (player.hasEffect(EffectRegistry.DRUNKENNESS.get())) {
                ...
                ArrayList<Component> components = new ArrayList<>(event.getComponent().getSiblings());

                components.add(0, event.getComponent());
                ...
                for (Component entry : components) {
                    ...
                    for (String word : entry.getString().split(" ")) {
                        if (!isNotNickname && !Arrays.asList(event.getMessage().split(" ")).contains(word)) {
                            result.add(word);
                    ...
                    component.append(new TextComponent(String.join(" ", result.toArray(new String[]{}))).withStyle(entry.getStyle()));
                }
